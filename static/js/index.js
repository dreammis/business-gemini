...
async function loadConfig() {
    try {
        ...
        document.getElementById('deleteSessionAfterChat').checked = configData.delete_session_after_chat !== false;
...
async function saveSettings() {
    ...
    const deleteSessionAfterChat = document.getElementById('deleteSessionAfterChat').checked;
    try {
        const res = await apiFetch(`${API_BASE}/api/config`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                ...
                delete_session_after_chat: deleteSessionAfterChat
            })
        });
        if (!res.ok) throw new Error((await res.json()).detail);
